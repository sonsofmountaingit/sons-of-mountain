...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) {
    ...
    return { title: `${t.destination_page.trip_singular} — ${t.destination_page.brand_name}` }
  }
...
async function TripContent({ params }: Props) {
  ...
  const galleryRaw = t.gallery as { image: { url?: string | null; alt?: string } | null; alt?: string }[] | null
    ...
    <article>
      ...
      <HeroSection
        ...
        heroGallery={(() => {
          ...
          if (fromHeroGallery.length) return fromHeroGallery
          return (galleryRaw ?? [])
            .filter((g) => g.image?.url)
            .map((g) => ({ url: mediaUrl(g.image!.url)!, alt: g.alt ?? g.image?.alt ?? undefined }))
        })()}
      ...
      <TravelTransportSection
        ...
        transportMapLink={t.transportMapLink as string | null}
      ...
      <BookingCtaSection
        ...
        itemType="trip"
        itemId={String(trip.id)}
        bookingStep1={t.bookingStep1 as string | null}
        bookingStep2={t.bookingStep2 as string | null}
        bookingStep3={t.bookingStep3 as string | null}
      />
