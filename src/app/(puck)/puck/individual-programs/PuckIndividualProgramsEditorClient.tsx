...
async function saveIndividualProgramsPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/individual-programs', {
    method: 'PATCH',
...
function IframeStyleInjector({ document: iframeDoc, children }: { document: Document | null; children: React.ReactNode }) {
  useEffect(() => {
    ...
    style.textContent = `
      :root { --color-bg: #0a0a0a; --color-text: #f5f5f5; }
...
export function PuckIndividualProgramsEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveIndividualProgramsPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveIndividualProgramsPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      initialHistory={{
        histories: [{ id: 'initial', state: { data: initialData } as any }],
        index: 0,
        appendData: false,
      }}
      headerTitle={saving ? 'Individual Programs — Saving…' : 'Individual Programs'}
      headerPath="/admin/globals/individual-programs-page"
      ...
      overrides={{
        ...
        headerActions: ({ children }) => (
          <>
            <a
              href="/admin/globals/individual-programs-page"
