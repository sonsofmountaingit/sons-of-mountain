...
async function save(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/stories', {
    method: 'PATCH',
...
export function PuckStoriesEditorClient({ initialData }: { initialData: Data }) {
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Stories — Saving…' : 'Stories Page'}
      headerPath="/admin/globals/stories-page"
      ...
      overrides={{
        headerActions: ({ children }) => (
          <>
            <a href="/admin/globals/stories-page" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, color: 'inherit', textDecoration: 'none', borderRadius: 4, border: '1px solid rgba(0,0,0,0.18)', marginRight: 6, opacity: 0.75 }}>← Admin</a>
            {children}
          </>
        ),
      }}
