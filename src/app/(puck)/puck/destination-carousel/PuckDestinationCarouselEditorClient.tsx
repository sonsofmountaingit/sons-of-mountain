...
async function saveCarouselPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/destination-carousel', {
    method: 'PATCH',
...
export function PuckDestinationCarouselEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveCarouselPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveCarouselPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Destination Carousel — Saving…' : 'Destination Carousel'}
      headerPath="/admin/globals/destination-carousel"
      ...
      overrides={{
        ...
        headerActions: ({ children }) => (
          <>
            <a
              href="/admin/globals/destination-carousel"
