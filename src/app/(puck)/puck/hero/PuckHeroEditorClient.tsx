...
async function saveHeroPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/hero', {
    method: 'PATCH',
...
export function PuckHeroEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveHeroPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveHeroPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Hero — Saving…' : 'Hero'}
      headerPath="/admin/globals/hero"
