...
import { Puck, resolveAllData, type Data, blocksPlugin, fieldsPlugin, outlinePlugin } from '@puckeditor/core'
...
async function save(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/home', {
    method: 'PATCH',
...
export function PuckHomeEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    timer.current = setTimeout(() => { save(appState.data).catch(() => {}) }, AUTOSAVE_DEBOUNCE_MS)
  }, [])
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Home — Saving…' : 'Home Page'}
      headerPath="/admin/globals/home-page"
      ...
      viewports={[
        ...
        { width: 768, height: 1024, label: 'Tablet', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="2" width="14" height="20" rx="2" /><circle cx="12" cy="18" r="1" fill="currentColor" /></svg> },
      ...
      overrides={{
        headerActions: ({ children }) => (
          <>
            <a href="/admin/globals/home-page" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, color: 'inherit', textDecoration: 'none', borderRadius: 4, border: '1px solid rgba(0,0,0,0.18)', marginRight: 6, opacity: 0.75 }}>← Admin</a>
            {children}
          </>
        ),
      }}
