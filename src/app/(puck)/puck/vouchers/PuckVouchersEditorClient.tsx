...
async function saveVouchersPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/vouchers', {
    method: 'PATCH',
...
export function PuckVouchersEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveVouchersPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveVouchersPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Vouchers — Saving…' : 'Vouchers'}
      headerPath="/admin/globals/vouchers"
