...
async function save(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/calendar', {
    method: 'PATCH',
...
export function PuckCalendarEditorClient({ initialData }: { initialData: Data }) {
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Calendar — Saving…' : 'Calendar Page'}
      headerPath="/admin/globals/calendar-page"
      ...
      overrides={{
        headerActions: ({ children }) => (
          <>
            <a href="/admin/globals/calendar-page" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, color: 'inherit', textDecoration: 'none', borderRadius: 4, border: '1px solid rgba(0,0,0,0.18)', marginRight: 6, opacity: 0.75 }}>← Admin</a>
            {children}
          </>
        ),
      }}
