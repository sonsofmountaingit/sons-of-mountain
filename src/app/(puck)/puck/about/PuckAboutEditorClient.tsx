...
async function saveAboutPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/about', {
    method: 'PATCH',
...
export function PuckAboutEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveAboutPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveAboutPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'About — Saving…' : 'About'}
      headerPath="/admin/globals/about"
