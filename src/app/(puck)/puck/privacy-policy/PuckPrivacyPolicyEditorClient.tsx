...
async function saveData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/privacy-policy', {
    method: 'PATCH',
...
export function PuckPrivacyPolicyEditorClient({ initialData }: { initialData: Data }) {
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Политика за поверителност — Saving…' : 'Политика за поверителност'}
      headerPath="/admin/globals/privacy-policy"
      ...
      overrides={{
        ...
        headerActions: ({ children }) => (
          <>
            <a
              href="/admin/globals/privacy-policy"
