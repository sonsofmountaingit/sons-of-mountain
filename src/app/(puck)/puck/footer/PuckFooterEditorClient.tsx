...
async function saveFooterPuckData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/footer', {
    method: 'PATCH',
...
function IframeStyleInjector({ document: iframeDoc, children }: { document: Document | null; children: React.ReactNode }) {
  useEffect(() => {
    ...
    style.textContent = `
      :root { --color-bg: #0a0a0a; --color-text: #f5f5f5; --color-accent: #e8d5b0; }
...
const EMPTY_DATA: Data = {
  content: [{ type: 'FooterBlock', props: { id: 'footer-block' } }],
  root: { props: {} },
}

export function PuckFooterEditorClient({ initialData }: { initialData: Data }) {
  ...
  const handlePublish = useCallback(async (data: Data) => {
    ...
    try {
      ...
      const ok = await saveFooterPuckData(resolved)
  ...
  const handleAction = useCallback((action: { type: string }, appState: { data: Data }) => {
    ...
    autosaveTimer.current = setTimeout(() => {
      saveFooterPuckData(appState.data).catch(() => {})
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Footer — Saving…' : 'Footer'}
      headerPath="/admin/globals/footer"
