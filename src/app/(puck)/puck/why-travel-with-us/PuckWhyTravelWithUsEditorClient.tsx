...
async function saveData(data: Data): Promise<boolean> {
  const res = await fetch('/api/puck/why-travel-with-us', {
    method: 'PATCH',
...
export function PuckWhyTravelWithUsEditorClient({ initialData }: { initialData: Data }) {
  ...
  return (
    <Puck
      ...
      headerTitle={saving ? 'Why Travel With Us — Saving…' : 'Why Travel With Us'}
      headerPath="/admin/globals/why-travel-with-us"
      ...
      overrides={{
        ...
        headerActions: ({ children }) => (
          <>
            <a href="/admin/globals/why-travel-with-us" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, color: 'inherit', textDecoration: 'none', borderRadius: 4, border: '1px solid rgba(0,0,0,0.18)', marginRight: 6, opacity: 0.75 }}>
              ← Admin
            </a>
            {children}
          </>
