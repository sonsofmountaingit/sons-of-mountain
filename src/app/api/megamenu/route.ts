...
const getMegamenuData = unstable_cache(
  async () => {
    ...
    const [bulgariaDests, abroadDests, bulgariaTrips, abroadTrips, individualTrips, bulgariaPrograms, abroadPrograms, individualPrograms] = await Promise.all([
      ...
      payload.find({
        collection: 'programs',
        where: { and: [{ navSection: { equals: 'bulgaria' } }, { status: { not_equals: 'draft' } }] },
        limit: 20,
      ...
      payload.find({
        collection: 'programs',
        where: { and: [{ navSection: { equals: 'abroad' } }, { status: { not_equals: 'draft' } }] },
        limit: 20,
        select: { title: true, slug: true, heroImage: true, price: true, currency: true, spotsAvailable: true },
        depth: 1,
      }),
      payload.find({
        collection: 'programs',
        where: { and: [{ navSection: { equals: 'individual' } }, { status: { not_equals: 'draft' } }] },
        limit: 20,
        select: { title: true, slug: true, heroImage: true, price: true, currency: true, spotsAvailable: true },
        depth: 1,
      }),
