  ...
  async (): Promise<CarouselData> => {
    try {
      ...
      if (carousel.destinationSource === 'manual' && manualItems.length) {
        ...
        const { docs } = await payload.find({ collection: 'destinations', where: { bookingStatus: { not_equals: 'archived' }, _status: { equals: 'published' } }, limit: 50, sort: 'name', depth: 2, overrideAccess: true })
